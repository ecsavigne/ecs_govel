...

	{
		...
		if TYPE_SERVICES == "" {
			...
			case "rest":
				...
				configlog.Sub("configs").Debugf("prepare_env HTTP_SERVER_Port: %s\n", HTTP_SERVER_PORT)
			default:
				configlog.Sub("configs").Errorf("TYPE_SERVICES is not valid view in app.env, values possible: [grpc, rest]. Actual value: %s\n", strings.ToLower(TYPE_SERVICES))
				os.Exit(2)
			}
		}
