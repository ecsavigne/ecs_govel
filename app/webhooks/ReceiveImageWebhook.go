...
func ReceiveImageWebhook(vInfo types.MessageInfo, vMessage *proto.Message, companyWhatsapp string, wac *whatsmeow.Client, Dns string) {
	// 0. Load env vars
	...
	text := vMessage.ImageMessage.GetCaption()
	ResponseMessageId := ""
	if vMessage.ImageMessage.ContextInfo != nil {
		ResponseMessageId = vMessage.ImageMessage.ContextInfo.GetStanzaId()
	}
	if vInfo.IsFromMe {
		Source = 3
	...
	defer func() {
		if err := recover(); err != nil {
			logMessage := "Client " + companyWhatsapp + ": Exception in ReceiveImageWebhook for " + contactJid + ". Interface in defer is: " + fmt.Sprintf("%+v", err)
			logg.Log("[ReceiveImageWebhook-ReceiveImageWebhook] ", companyWhatsapp, contactJid, logMessage, true)
			return
		}
	}()
	...
	// 4. Download image file from Whatsapp
	localPath, filename, clientOriginalName, idParentDrive, idFileDrive := "", "", "", "", ""
	body := &bytes.Buffer{}
	writer := multipart.NewWriter(body)
	textoMessage := ""
	if Source != 3 {
		localPath, filename, clientOriginalName, idParentDrive, idFileDrive, _ = DownloadImageFromWPP(vInfo, vMessage, companyWhatsapp, wac, contactJid)
		...
		text = ""
		textoMessage = "Uma imagem foi enviada por você desde outro dispositivo. Pode visualizá-la no aplicativo ou no whatsapp web."
	}
	...
	*/
	...
	err = writer.Close()
	if err != nil {
		logMessage := "Client " + companyWhatsapp + ": Is receiving an Image message from contactJid: " + contactJid + " but an error occurr creating form-data to send through the Webhook to Laravel app. Msgid is: " + msgid + ". Error was: " + err.Error()
		panic(logMessage)
	}
	...
	if err != nil {
		...
		logMessage := "Client " + companyWhatsapp + ": Error sending ImageMessage of contactJid " + contactJid + " from Webhook to LaravelApp. Msgid is: " + msgid + ". Error was: " + err.Error()
		panic(logMessage)
	} else {
		boddy, _ := io.ReadAll(resp.Body)
		serverresponse := string(boddy)
		logMessage := "Client " + companyWhatsapp + ": Sended ImageMessage of contactJid " + contactJid + " from Webhook to LaravelApp successfully.  Msgid is: " + msgid + ". Path is " + localPath + ". Serverresponse is " + serverresponse
		logg.Log("[ReceiveImageWebhook-ReceiveImageWebhook] ", companyWhatsapp, contactJid, logMessage, false)
	}
	defer resp.Body.Close()
}

// DownloadImageFromWPP : saves audio into storage and retrieves filepath & error
func DownloadImageFromWPP(vInfo types.MessageInfo, vMessage *proto.Message, companyWhatsapp string, wac *whatsmeow.Client, contactJid string) (string, string, string, string, string, error) {
	//  0. Get datas
	mimetype := *vMessage.ImageMessage.Mimetype
	ext := helpers.DocHandlerExts[mimetype]

	// 1. Capture exceptions
	defer func() {
		if err := recover(); err != nil {
			logMessage := "Client " + companyWhatsapp + ": Recovered from Exception when downloading an Image file from Whsatsapp. Interface in defer is: " + fmt.Sprintf("%+v", err)
			logg.Log("[ReceiveImageWebhook-DownloadImageFromWPP] ", companyWhatsapp, contactJid, logMessage, true)
		}
	}()

	// 2. Verify valid client
	if wac == nil {
		logMessage := "Client " + companyWhatsapp + ": Wac is nil when downloading an Image file from Whsatsapp."
		panic(logMessage)
	}

	// 3. Downlaod audio file from whatsapp
	data, err := wac.Download(vMessage.ImageMessage)
	if err != nil {
		logMessage := "Client " + companyWhatsapp + ": An error ocurr when downloading an Image file from Whsatsapp." + " Error was: " + err.Error()
		panic(logMessage)
	}

	// 4. Save file temporarily in local storage
	helpers.CreateDirIfNotExist(os.Getenv("MESSAJE_FILES") + "/image/" + contactJid)
	filepath := fmt.Sprintf(os.Getenv("MESSAJE_FILES")+"/image/%s/%s.%s", contactJid, vInfo.ID, ext)
	...
	logMessage := "Client " + companyWhatsapp + ": Has downloaded the Image file from Whsatsapp fom contactJid: " + contactJid + ". Filepath :" + filepath
	logg.Log("[ReceiveImageWebhook-DownloadImageFromWPP] ", companyWhatsapp, contactJid, logMessage, false)
	clientOriginalName := ""
