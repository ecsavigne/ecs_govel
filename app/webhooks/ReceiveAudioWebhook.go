...
func ReceiveAudioWebhook(vInfo types.MessageInfo, vMessage *proto.Message, companyWhatsapp string, wac *whatsmeow.Client, Dns string) {
	// 0. Load env vars
	...
	ResponseMessageId := ""
	if vMessage.AudioMessage.ContextInfo != nil {
		ResponseMessageId = vMessage.AudioMessage.ContextInfo.GetStanzaId()
	}
	...
	defer func() {
		if err := recover(); err != nil {
			logMessage := "Client " + companyWhatsapp + ": Exception in ReceiveAudioWebhook for " + contactJid + ". Interface in defer is: " + fmt.Sprintf("%+v", err)
			logg.Log("[ReceiveAudioWebhook-ReceiveAudioWebhook] ", companyWhatsapp, contactJid, logMessage, true)
		}
	}()
	...
	textoMessage := ""
	if Source != 3 {
		localPath, filename, clientOriginalName, idParentDrive, idFileDrive, _ = DownloadAudioFromWPP(vInfo, vMessage, companyWhatsapp, wac, contactJid)
		...
		textoMessage = "Uma mensagem de áudio foi enviada por você desde outro dispositivo. Pode visualizá-la no aplicativo ou no whatsapp web."
	}
	...
	*/
	...
	_ = writer.WriteField("type", "audio")
	...
	err = writer.Close()
	if err != nil {
		logMessage := "Client " + companyWhatsapp + ": Is receiving an Audio message from contactJid: " + contactJid + " but an error occurr creating form-data to send through the Webhook to Laravel app. Msgid is: " + msgid + ". Error was: " + err.Error()
		panic(logMessage)
	}
	...
	if err != nil {
		...
		logMessage := "Client " + companyWhatsapp + ": Error sending AudiotMessage of contactJid " + contactJid + " from Webhook to LaravelApp. Msgid is: " + msgid + ". Error was: " + err.Error()
		panic(logMessage)
	} else {
		boddy, _ := io.ReadAll(resp.Body)
		serverresponse := string(boddy)
		logMessage := "Client " + companyWhatsapp + ": Sended AudioMessage of contactJid " + contactJid + " from Webhook to LaravelApp successfully.  Msgid is: " + msgid + ". Path is " + localPath + ". Serverresponse is " + serverresponse
		logg.Log("[ReceiveAudioWebhook-ReceiveAudioWebhook] ", companyWhatsapp, contactJid, logMessage, false)
	}
	defer resp.Body.Close()
}

// DownloadAudioFromWPP : saves audio into storage and retrieves filepath & error
func DownloadAudioFromWPP(vInfo types.MessageInfo, vMessage *proto.Message, companyWhatsapp string, wac *whatsmeow.Client, contactJid string) (string, string, string, string, string, error) {
	//  0. Get datas
	mimetype := *vMessage.AudioMessage.Mimetype
	ext := helpers.DocHandlerExts[mimetype]

	// 1. Capture exceptions
	defer func() {
		if err := recover(); err != nil {
			logMessage := "Client " + companyWhatsapp + ": Recovered from Exception when downloading an Audio file from Whsatsapp. Interface in defer is: " + fmt.Sprintf("%+v", err)
			logg.Log("[ReceiveAudioWebhook-DownloadAudioFromWPP] ", companyWhatsapp, contactJid, logMessage, true)
		}
	}()

	// 2. Verify valid client
	if wac == nil {
		logMessage := "Client " + companyWhatsapp + ": Wac is nil when downloading an Audio file from Whsatsapp."
		logg.Log("[ReceiveAudioWebhook-DownloadAudioFromWPP] ", companyWhatsapp, contactJid, logMessage, true)
		panic(logMessage)
	}

	// 3. Downlaod audio file from whatsapp
	data, err := wac.Download(vMessage.AudioMessage)
	if err != nil {
		logMessage := "Client " + companyWhatsapp + ": An error ocurr when downloading an Audio file from Whsatsapp." + " Error was: " + err.Error()
		logg.Log("[ReceiveAudioWebhook-DownloadAudioFromWPP] ", companyWhatsapp, contactJid, logMessage, true)
		panic(logMessage)
	}

	// 4. Save file temporarily in local storage
	helpers.CreateDirIfNotExist(os.Getenv("MESSAJE_FILES") + "/audio/" + contactJid)
	filepath := fmt.Sprintf(os.Getenv("MESSAJE_FILES")+"/audio/%s/%s.%s", contactJid, vInfo.ID, ext)
	...
	logMessage := "Client " + companyWhatsapp + ": Has downloaded the Audio file from Whsatsapp fom contactJid: " + contactJid + ". Filepath :" + filepath
	logg.Log("[ReceiveAudioWebhook-DownloadAudioFromWPP] ", companyWhatsapp, contactJid, logMessage, false)
	clientOriginalName := ""
