...
func (c *Document) SendDocumentMessage(w http.ResponseWriter, r *http.Request) {
	logg.GeneralLogger.Println(r.FormValue("RequestOrigin"), "-SendDocumentMessage")
	...
	defer func() {
		if err := recover(); err != nil {
			logMessage := "Client " + vars["CompanyPhone"] + ": Exception sending document message in DocumentController-SendDocumentMessage " + r.FormValue("RemoteJid") + ". Interface in defer is: " + fmt.Sprintf("%+v", err)
			logg.Log("[DocumentController-SendDocumentMessage] ", vars["CompanyPhone"], "", logMessage, true)
	...
	if err != nil {
		panic("Missing Document file in request")
	}
	...
	chat := migrations.Chats{
		...
		ResponseMessageText: r.FormValue("Caption"),
	...
	// 5. send message
	response := repositories.DocumentRepository.SendDocumentMessage(chat, file, serviceDate)
