...
func (dr *DocumentRepositories) SendDocumentMessage(chat migrations.Chats, file multipart.File, serviceDate string) structs.Response {
	...
	uploaded, err := structs.Connections[chat.CompanyPhone].Upload(context.Background(), data, whatsmeow.MediaDocument)
	if err != nil {
		panic("Failed uploading document file to Whatsapp Server: " + fmt.Sprintf("%v", path) + ": " + fmt.Sprintf("%v", err))
	}
	re := regexp.MustCompile(`(.[^.]+)$`)
	// 3. prepare to send message
	wppmsg := &waProto.Message{DocumentMessage: &waProto.DocumentMessage{
		...
		FileName:      proto.String(chat.ClientOriginalName),
		Title:         proto.String(re.Split(chat.ClientOriginalName, -1)[0]),
	}}
	if chat.ResponseMessageID != "" {
		...
		wppmsg.DocumentMessage.ContextInfo = &waProto.ContextInfo{
	...
	msgid := timer.ID
	if err != nil {
		panic("Error sending document. Error was:" + err.Error())
	} else {
	...
	logMessage := "Client " + chat.CompanyPhone + ": Document message sended successfully to ContactPhone " + chat.ContactPhone
	logg.Log("[DocumentRepository-SendDocumentMessage]", chat.CompanyPhone, chat.ContactPhone, logMessage, false)
	...
	chat.Path = "https://drive.google.com/uc?export=view&id=" + mimetype //+ fileUploadId
