...
// // Repositories Nombre
// func (a *NombreRepositories) SendNombreMessage(chat migrations.Chats, file multipart.File, serviceDate string) structs.Response {
// 	// 1. save file into storage
// 	path, mimetype, err := helpers.StoreMessageFile(file, chat.ClientOriginalName, chat.ContactPhone)
// 	if err != nil {
// 		panic("[NombreMessageRepository-SendNombreMessage] - %s " + "Acontecio um erro salvando ")
// 	}

// 	// 2. upload file to whatsapp server
// 	data, _ := os.ReadFile(path)
// 	uploaded, err := structs.Connections[chat.CompanyPhone].Upload(context.Background(), data, whatsmeow.MediaNombre)
// 	if err != nil {
// 		panic("[NombreMessageRepository-SendNombreMessage] " + " Failed uploading Nombre file to Whatsapp Server: " + fmt.Sprintf("%v", path) + " " + fmt.Sprintf("%v", err))
// 	}

// 	// 3. prepare to send message
// 	wppmsg := &waProto.Message{NombreMessage: &waProto.NombreMessage{
...
// 		}
// 		wppmsg.NombreMessage.ContextInfo = &waProto.ContextInfo{
...
// 	//whatsmeow.SendResponse
// 	timer, err := structs.Connections[chat.CompanyPhone].SendMessage(context.Background(), ContactJID, wppmsg)
// 	// msgid := whatsmeow.GenerateMessageID()
// 	msgid := timer.ID
// 	if err != nil {
// 		panic("[NombreMessageRepository-SendNombreMessage] - %s %s \n" + "Error sending Nombre. Error was: " + err.Error())
// 	} else {
...
// 	logMessage := "Client " + chat.CompanyPhone + ": Nombre message sended successfully in  ContactPhone " + chat.ContactPhone
// 	logg.Log("[NombreMessageRepository-SendNombreMessage]", chat.CompanyPhone, chat.ContactPhone, logMessage, false)
