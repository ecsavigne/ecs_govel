...
// Repositories Audio
func (a *AudioRepositories) SendAudioMessage(chat migrations.Chats, file multipart.File, serviceDate string) structs.Response {
	// 1. save file into storage
	path, mimetype, err := helpers.StoreMessageFile(file, chat.ClientOriginalName, chat.ContactPhone)
	if err != nil {
		panic("[AudioMessageRepository-SendAudioMessage] - %s " + "Acontecio um erro salvando ")
	}

	// 2. upload file to whatsapp server
	data, _ := os.ReadFile(path)
	uploaded, err := structs.Connections[chat.CompanyPhone].Upload(context.Background(), data, whatsmeow.MediaAudio)
	if err != nil {
		panic("[AudioMessageRepository-SendAudioMessage] " + " Failed uploading audio file to Whatsapp Server: " + fmt.Sprintf("%v", path) + " " + fmt.Sprintf("%v", err))
	}

	// 3. prepare to send message
	wppmsg := &waProto.Message{AudioMessage: &waProto.AudioMessage{
	...
	msgid := timer.ID
	if err != nil {
		panic("[AudioMessageRepository-SendAudioMessage] - %s %s \n" + "Error sending audio. Error was: " + err.Error())
	} else {
	...
	logMessage := "Client " + chat.CompanyPhone + ": Audio message sended successfully in  ContactPhone " + chat.ContactPhone
	logg.Log("[AudioMessageRepository-SendAudioMessage]", chat.CompanyPhone, chat.ContactPhone, logMessage, false)
